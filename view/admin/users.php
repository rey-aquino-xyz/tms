...
<script>
    $(document).ready(function () {
        ...
        $('[name= "add-taxpayer-link"]').on('click', function (e) {
            e.preventDefault();
            $("#main-content").load('add-user.php');
        })

        $('[name="edit-info-link"]').on('click', function (e) {
            e.preventDefault();
            var userid = $(this).attr('href');
            $("#main-content").load('view-user.php', { user_id: userid });
        })

        $('[name="view-transction-link"]').on('click', function (e) {
            e.preventDefault();
            var userid = $(this).attr('href');
            $("#main-content").load('view-user-transaction.php', { userid: userid });
        })
    });
</script>
    ...
    <table class="table table-striped" name="sampple-tbl">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?=UserController::GetByUserId($r['user_id'])->Contact?></td>
                <td><?=UserController::GetByUserId($r['user_id'])->Barangay . ', ' . UserController::GetByUserId($r['user_id'])->Municipality . ', ' . UserController::GetByUserId($r['user_id'])->Province?>
                </td>
                <td>
                    <div class="d-flex">
